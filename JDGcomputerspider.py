...
def main():
    ...
    # 爬取前两页的所有笔记本
    for i in range(0,1):
        #每页地址规律：http://list.jd.om/list.html?cat=......
        print("正在爬取第"+str(i+1)+"页的信息...")
        url='https://list.jd.com/list.html?cat=670,671,672&page='+str(i+1)   #没网的日子可真是太难受了！！手打地址很酸爽
        get_page_data(url,allNames,allCommentNums,allPrice,allShops)
    ...
    prices=[]
    for name in names:
        if allPrice[name]==None:
            prices.append(0)
        else:
            prices.append(eval(allPrice[name]))
...
def get_page_data(url,allNames,allCommentNums,allPrice,allShops):
    ...
    for id in ids:
        str = str + id + ','
    commentJS_url = 'https://club.jd.com/comment/productCommentSummaries.action?my=pinglun&referenceIds=' + str[
                                                                                                            :-1] + '&callback=jQuery5043746'
    # 爬取该js包，获取每台电脑的评论数
    ...
    pat = '{(.*?)}'
    priceStr = re.compile(pat).findall(data)  # priceStr用来存放每个商品的关于价格方面的所有信息
    prices = {}
    for id in ids:
        for str in priceStr:
            if id in str:
    ...
    for i in range(0,len(ids)):
        if prices[ids[i]]=='':
            allPrice[names[i]]=None
        else:
            allPrice[names[i]]=prices[ids[i]]
    #名字和店铺联系起来
    for i in range(0,len(ids)):
...
def tiaoxing(names,prices):
    bar=Bar("笔记本电脑价格图","X_电脑名,Y_价格")  #需要导包
